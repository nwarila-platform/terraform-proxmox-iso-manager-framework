...
2. **Renovate rangeStrategy check.** The shared org baseline at `nwarila-platform/.github/.github/renovate.json5` MUST set `terraform.rangeStrategy: "pin"`. Repo-local overrides MAY narrow this for a specific manager but MUST NOT widen it without a superseding repo-level ADR. A CI script MAY assert this.
...
- The exact-pin policy applies only inside `nwarila-platform`. Future external consumers (if any) inherit the strictness; if their context demands range constraints they fork or pin internally.
...
1. The `nwarila-platform` org continues to consume Terraform modules primarily from itself, not from third-party authors with conflicting version constraints.
...
3. Consumers of `nwarila-platform` Terraform modules are willing to accept the discipline of running the exact pinned CLI version.
